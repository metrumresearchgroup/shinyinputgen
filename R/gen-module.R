...
gen_ui_input <- function(.input, .input_name) {
  if (.input$type == "numeric") {
    return(glue::glue('    numericInput(shiny::NS(id, "{.input_name}"), "{.input$label}", {.input$value}, min = {.input$min}, max = {.input$max})'))
  }
  if (.input$type == "text") {
    return(glue::glue('    textInput(shiny::NS(id, "{.input_name}"), "{.input$label}", {.input$value})'))
  }
  if (.input$type == "checkbox") {
    return(glue::glue('    checkboxInput(shiny::NS(id, "{.input_name}"), "{.input$label}", {.input$value})'))
  }
  if (.input$type == "select") {
    vals <- glue::glue_collapse(glue::glue('"{.input$values}" = "{.input$values}"'), sep = ",\n")
    return(glue::glue('    selectInput(shiny::NS(id, "{.input_name}"), "{.input$label}", c(\n',
                      vals, ')\n)'))
  }
